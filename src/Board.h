...
public:
	...
	Board() {
		pieces = vector<Piece>({
			Piece(string("red"), 0, pointSet({
					  Point(0,0), Point(1,0),
					  Point(0,1), Point(1,1), Point(2,1),
					  Point(0,2), Point(1,2)
					  })),
			Piece(string("yellow"), 1, pointSet({
					  Point(0,0), Point(1,0),
								  Point(1,1), Point(2,1),
											  Point(2,2)
					  })),
			Piece(string("green"), 2, pointSet({
								  Point(1,0), Point(2,0),
					  Point(0,1), Point(1,1), Point(2,1),
								  Point(1,2), Point(2,2), Point(3,2)
					  })),
			Piece(string("dark green"), 3, pointSet({
					  Point(0,0),
					  Point(0,1),
					  Point(0,2),
					  Point(0,3),
			...
			Piece(string("light orange"), 4, pointSet({
					  Point(0,0),
					  Point(0,1),
					  })),
			Piece(string("dark orange"), 5, pointSet({
					              Point(1,0),
					  Point(0,1), Point(1,1),
					  Point(0,2), Point(1,2),
							      Point(1,3)
					  })),
			Piece(string("violet"), 6, pointSet({
					  Point(0,0), 
					  Point(0,1), Point(1,1),
			...
			Piece(string("sky blue"), 8, pointSet({
	...
	void placePiece(int pieceIndex, int rotationIndex, Point position) {
		if (placedPieces.count(pieceIndex) || !canPlacePiece(pieceIndex, rotationIndex, position))
			throw exception("Piece already exists. Please use canPlacePiece to check validity");
